...
export default class App {
    ...
    constructor() {
        ...
        this.memory = new Memory((<any>window).engine.instance.exports.ec);
        (<any>window).memory = this.memory;
        (<HTMLButtonElement>html.get(`#button_memory`)).addEventListener(
            "pointerdown",
            () => {
                const address = (<HTMLInputElement>html.get(`#input_memory`))
                    .value;
                const data = this.memory.read4byte(parseInt(address, 16));
                (<HTMLSpanElement>(
                    html.get(`#text_memory`)
                )).innerHTML = `${data.hex}<br>${data.int}<br>${data.float}`;
            }
        );
